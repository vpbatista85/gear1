...
if arquivos:
    ...
    # Pegamos o ID correspondente ao arquivo selecionado
    arquivo_id = next((arq['id'] for arq in arquivos if arq['name'] == nome_arquivo), None)

    if arquivo_id:
        # Sempre baixa novamente o arquivo (sem cache)
        ...
        st.dataframe(df.tail(10))  # Últimas 10 linhas

        # Aplica refresh automático SOMENTE após a seleção
        auto_refresh(3000)
