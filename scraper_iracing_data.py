...
semaphore = asyncio.Semaphore(12)  # Limita 10 requisições simultâneas
...
async def verificar_iracing_async(piloto, tentativa=0):
    """Obtém os dados do piloto de forma assíncrona, com retry."""
    async with semaphore:
        try:
            return verificar_iracing(piloto)
            ...
            if tentativa < MAX_RETRIES:
                ...
                return await verificar_iracing_async(piloto, tentativa + 1)
...
async def processar_batch(pilotos_batch):
    ...
    tasks = [verificar_iracing_async(piloto) for piloto in pilotos_batch]
